...
import {assert, expect} from 'chai'; // Using Assert style // Using Expect style
...
const sandBoxTests = async () => {
  ...
  if (readToken && writeToken && deleteToken) {
    ...
    await sb.delete(deleteToken, '~.*', {}, '', '', true).then(() => {
    }).catch(err => {
